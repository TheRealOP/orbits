...
This document explains how Codex reports token usage through the app-server protocol and how Orbit should account for it.
...
Orbit sees these events wrapped inside the app-server message stream.
...
For Orbit, `model_context_window` should be displayed or logged separately from spend.

## Recommended Accounting Strategy For Orbit
...
## What Orbit Should And Should Not Do
...
- Expect one thread to span multiple turns when Orbit reuses a live Codex thread.
...
## Practical Interpretation For Orbit Logs
...
That is a strong signal for Orbit:

- use absolute totals as the main accounting surface
- ignore last/delta values for totals

## Recommended Orbit Documentation Contract

If Orbit documents token reporting externally, the contract should be:

- Live token totals come from Codex thread-scoped cumulative usage.
- Incremental usage may also be emitted, but Orbit does not use it for totals.
