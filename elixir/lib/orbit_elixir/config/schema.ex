defmodule OrbitElixir.Config.Schema do
  @moduledoc false
  ...
  defmodule Workspace do
    ...
    @primary_key false
    embedded_schema do
      field(:root, :string, default: Path.join(System.tmp_dir!(), "orbit_workspaces"))
    end
  ...
  defmodule Agent do
    ...
    embedded_schema do
      ...
      field(:provider_routes, {:array, :map}, default: [])
    end
    ...
    def changeset(schema, attrs) do
      ...
      |> validate_required([:default_provider])
  ...
  defp finalize_settings(settings) do
    ...
    workspace = %{
      settings.workspace
      | root: resolve_path_value(settings.workspace.root, Path.join(System.tmp_dir!(), "orbit_workspaces"))
    }
    ...
    agent = %{
      settings.agent
      | provider_routes: normalize_provider_routes(settings.agent.provider_routes)
    }
    ...
    %{
      ...
      | tracker: tracker,
        ...
        agent: agent,
        codex: codex,
        providers: normalize_keys(settings.providers)
    }
  end

  defp normalize_provider_routes(routes) when is_list(routes), do: Enum.map(routes, &normalize_keys/1)
  ...
  defp expand_local_workspace_root(_workspace_root) do
    Path.expand(Path.join(System.tmp_dir!(), "orbit_workspaces"))
  end
