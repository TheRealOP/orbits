...
Orbit is a long-running automation service that continuously reads work from an issue tracker
...
- Orbit is a scheduler/runner and tracker reader.
...
Orbit is easiest to port when kept in these layers:
...
- `root` (path string or `$VAR`)
  - Default: `<system-temp>/orbit_workspaces`
  - `~` is expanded.
...
- `workspace.root`: path resolved to absolute, default `<system-temp>/orbit_workspaces`
...
Startup MUST follow the targeted Codex app-server contract. Orbit additionally requires the
client to:
...
- Purpose: execute a raw GraphQL query or mutation against Linear using Orbit's configured
...
- Reuse the configured Linear endpoint and auth from the active Orbit workflow/runtime config; do
...
Orbit does not require first-class tracker write APIs in the orchestrator.
...
- `GET /api/v1/<issue_identifier>`
  ...
  - Suggested response shape:
    ...
    ```json
    {
      ...
      "status": "running",
      "workspace": {
        "path": "/tmp/orbit_workspaces/MT-649"
      },
      "attempts": {
        "restart_count": 1,
      ...
      "logs": {
        "codex_session_logs": [
          {
            "label": "latest",
            "path": "/var/log/orbit/codex/MT-649/latest.log",
            "url": null
          }
        ]
      },
...
- `linear_graphql` client-side tool extension exposes raw Linear GraphQL access through the
  app-server session using configured Orbit auth.
...
This appendix describes a common extension profile in which Orbit keeps one central
